...
# Label and tidy ESBL isolate data
...
# Plot ST rates per 100 isolates per year
...
# Apply transformations to all columns except 'Isolate' so ARGs are present (1) or absent (0)
...
```{r}
# Add year breakdown to ARG plot

# Add year to ARG data
ARGs <- ARGs %>%
...
ARG_summary_data <- ARGs_year_long %>%
  group_by(ARG, Year) %>%
...
# Plot ARG rates per 100 isolates per year
...
# Apply transformations to all columns except 'Isolate' so plasmids are present (1) or absent (0)
...
# Mutate the data to have Isolate and Plasmid columns
...
# Print all unique plasmid replicons in data set
(colnames(plasmids)[-1])
```
...
# Apply transformations to all columns except 'Isolate' so gene names are present (1) or absent (0)
...
# Define IDs of strains found in 3 or more households
...
pvals <- data.frame(
  ...
  p.adj  = c(0.000924850, 1.000000000, 0.007547248),
...
```{r}
# Prepare the data (make household variable for isolates from strains found in 3+ households)
...
```{r}
# Load ST131 SNP distance matrix including recombination sites
